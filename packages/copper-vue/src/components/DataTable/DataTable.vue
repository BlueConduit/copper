<template>
  <Table>
          ...
          >
            <span class="cu-datatable-sort__flex">
              ...
              <svg
                ...
                height="16"
                fill="currentColor"
                class="cu-datatable-sort__icon--unsorted"
                viewBox="0 0 16 16"
              >
                <path
                  fill-rule="evenodd"
    ...
    <table-body>
      <table-row v-for="(row, row_index) in data" :key="row_index">
  ...
  <div class="cu-datatable__pagination" v-if="paginate">
    ...
    <div class="cu-datatable__pagination__right-controls">
      ...
      <span class="cu-datatable__pagination__page-nums">
        <span v-for="(label, index) in pageNumberButtons" :key="index">
          <button
            v-if="label !== '...'"
...
export default {
  ...
  emits: {
    ...
    changePage: (page) => {
      ...
      console.warn(
        "Invalid event payload, you must specify the page the table should change to. Valid choices are 'next', 'prev', or a specific page number as an int."
      );
      return false;
    },
    /**
    ...
    changePageSize: (pageSize) => {
      ...
      console.warn(
        "Invalid event payload, you must specify the new page size as an int."
      );
      return false;
    },
    /**
  ...
  setup(props) {
    ...
    const isNextDisabled = computed(
      () => props.index + props.pageSize >= props.totalSize
    );
    ...
    const currentPageNumber = computed(
      () => Math.floor(props.index / props.pageSize) + 1
    );
    ...
    const pageNumberButtons = computed(() => {
      ...
      // the list
      if (
        currentPageNumber.value <= 3 ||
        currentPageNumber.value >= lastPageNumber.value - 2
      ) {
        buttonLabels = [
          1,
          2,
