  ...
  setup() {
    ...
    const sortData = () => {
      ...
      const compare = (a, b) => {
        ...
        if (a[col] < b[col]) { return compareReturnVal; }
        if (a[col] > b[col]) { return compareReturnVal*-1; }
        return 0;
      }
    ...
    const updateTableSubset = () => {
      ...
      dataSubset.value = tableData.value.slice(index.value, index.value+pageSize.value)
    }
